...
def get_num_dependents(library_name, username, token):
    ...
    dependents_num_repos_class = dependents_div.find_all("div", class_="table-list-header-toggle states float-left pl-0")
    num_dependents = re.findall("""(?<=<\/svg>)([\s\S]*)(?=Repositories)""", dependents_num_repos_class[0].prettify())
...
while True:
    response = requests.get(next_dependents_page_url, auth=HTTPBasicAuth(username, token))
    if response.status_code == 429:
        # 429: too many requests, wait then continue
        retry_after = int(response.raw.headers._container['retry-after'][1]) + 10
        print(f"429 (too many requests): {next_dependents_page_url} retrying after {str(retry_after)}")
    ...
    dependent_href_div_class = dependents_div.find_all("div", class_="Box-row d-flex flex-items-center")
    dependent_repositories = list(map(lambda i:re.findall(DEPENDENTS_PATTERN1, i.prettify()), dependent_href_div_class)) 
    dependent_repositories = list(map(lambda i:re.findall(DEPENDENTS_PATTERN2, i[0]), dependent_repositories)) 
    num_dependents_retrieved += len(dependent_repositories)
    print(f'Progress: {num_dependents_retrieved}/{num_dependents} ({round(num_dependents_retrieved*100/num_dependents,2)}%)')
    ...
    next_url_matches = re.findall(NXT_BTN_PATTERN, button_div_class[0].prettify())
    if len(next_url_matches) == 0:
        # no next page with dependents, break the loop
        break
    next_dependents_page_url = BASE_URL + DEPENDENTS_PREFIX + re.findall(NXT_BTN_PATTERN, button_div_class[0].prettify())[0]
